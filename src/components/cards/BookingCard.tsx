...
const STYLES = {
  card: {
    base: "group rounded-2xl border border-border bg-card shadow-sm transition hover:shadow-md hover:ring-1 ring-brand",
    desktop: "w-full p-6",
    mobile: "w-[375px] p-5",
  },
  button: { 
    ...
    report:
      "text-[16px] font-bold leading-[24px] text-orange-5 hover:opacity-80 transition cursor-pointer",
  },
  text: {
...
const Avatar: React.FC<{
  ...
  size: number;
  showRing?: boolean;
}> = React.memo(({ src, alt, size, showRing = true }) => (
  <div
    className={`relative overflow-hidden rounded-full ${
      showRing ? "ring-1 ring-border" : ""
    }`}
...
const StatusBadge: React.FC<{ status: BookingStatus }> = React.memo(
  ({ status }) => {
    const config = STATUS_CONFIG[status];
    return (
      <div
        className={`inline-flex items-center gap-2 text-[12px] ${config.text}`}
      >
...
const FormattedDateTime: React.FC<{ dateTime: string }> = React.memo(
  ({ dateTime }) => {
...
const CallButton: React.FC<{
  ...
  disabled?: boolean;
  strong?: boolean;
}> = React.memo(({ onClick, disabled, strong }) => (
  <button
    type="button"
    onClick={onClick}
    disabled={disabled}
    aria-label="Call"
    className={`${STYLES.button.call} ${
...
const ChangeButton: React.FC<{
  ...
  label?: string;
  disabled?: boolean;
}> = React.memo(({ onClick, label = "Change", disabled }) => (
  <button
    type="button"
    onClick={onClick}
    disabled={disabled}
...
const PrimaryButton: React.FC<{
  ...
  <button
    ...
    onClick={onClick}
    disabled={disabled}
    className={`${STYLES.button.primary} ${className}`}
  >
    {children}
  </button>
));
...
const WaitingSection: React.FC<{
  ...
  const { message: messageAction, call: callAction } =
    useActionsLookup(actions);
  ...
  return (
    <div className={`mt-6 rounded-xl p-4 ring-1 ring-border ${cfg.bg}`}>
      <div className={`flex items-center ${isDesktop ? "" : "flex-col gap-3"}`}>
        {note && (
          <div
            className={`text-[14px] font-medium leading-[22px] text-muted-text ${
              isDesktop ? "grow" : ""
            }`}
          >
            {note}
        ...
        <div className="flex items-center gap-3">
          {messageAction && (
            <PrimaryButton
              ...
              className={isDesktop ? "" : "px-6 text-[14px]"}
            >
...
const SuccessSection: React.FC<{
  ...
  actions: BookingAction[];
  isDesktop: boolean;
}> = React.memo(({ successDate, actions, isDesktop }) => {
  const {
    report: reportAction,
    review: reviewAction,
    call: callAction,
    ...
    <div className="mt-6 rounded-xl p-4 ring-1 ring-green/60 bg-green-bg">
      <div className={`flex items-center ${isDesktop ? "" : "flex-col gap-3"}`}>
        <div className={isDesktop ? "grow" : ""}>
          <div className="text-[14px] font-medium leading-[24px] text-green">
            Success date:
          </div>
          <div className="text-[14px] font-medium leading-[24px] text-green">
            {successDate}
        ...
        <div className="flex items-center gap-6">
          {reportAction && (
            <button
              ...
              className={STYLES.button.report}
            >
...
const CanceledSection: React.FC<{
  actions: BookingAction[];
  isDesktop: boolean;
}> = React.memo(({ actions, isDesktop }) => {
  const { report: reportAction } = useActionsLookup(actions);
    ...
    <div className="mt-6 rounded-xl p-4 ring-1 ring-border bg-red-100">
      <div
        className={`flex items-center ${
          isDesktop ? "justify-between" : "flex-col gap-3"
        }`}
      >
...
CanceledSection.displayName = "CanceledSection";
...
const DesktopLayout: React.FC<BookingCardProps> = React.memo((props) => {
  ...
  return (
    <div className={`${STYLES.card.base} ${STYLES.card.desktop} ${className}`}>
          ...
          <div className="flex items-center justify-between">
            ...
            {status === "waiting" && changeAction && (
              <ChangeButton
                ...
                label={changeAction.label}
                disabled={changeAction.disabled}
              />
            )}
          </div>
      ...
      {status === "canceled" && (
        <CanceledSection actions={actions} isDesktop={true} />
      )}
    </div>
  );
});
...
const MobileLayout: React.FC<BookingCardProps> = React.memo((props) => {
  ...
  return (
    <div className={`${STYLES.card.base} ${STYLES.card.mobile} ${className}`}>
      <div className="flex flex-col gap-3">
        ...
        <div className="flex gap-3">
          ...
          <div className="flex flex-col">
            <div className="truncate text-[18px] leading-[24px] font-bold text-ink">
              {title}
            </div>
            <div className="truncate text-[14px] leading-[22px] font-medium text-ink">
              By {sitterName}
            </div>
          </div>
        </div>
      ...
      {status === "canceled" && (
        <CanceledSection actions={actions} isDesktop={false} />
      )}
    </div>
  );
});
