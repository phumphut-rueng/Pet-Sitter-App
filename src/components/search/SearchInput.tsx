...
export function SearchInput({
  ...
  const { suggestions, showSuggestions, selectSuggestion, hideSuggestions } = useSearchSuggestions({
    searchTerm: value,
  });
  ...
  return (
    <div className={className}>
      ...
      <div className="relative" ref={suggestionsRef}>
        <Search className={`absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-4 ${size === 'sm' ? 'h-4 w-4' : size === 'md' ? 'h-5 w-5' : 'h-6 w-6'
          }`} />
        {value && (
          <button
            onClick={() => onChange('')}
            className={`absolute right-8 top-1/2 transform -translate-y-1/2 text-gray-4 hover:text-gray-6 ${size === 'sm' ? 'h-4 w-4' : 'h-4 w-4'
              }`}
          >
            <X className="h-4 w-4" />
          </button>
          ...
          <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-2 rounded-lg shadow-lg z-50 max-h-60 overflow-y-auto">
            {suggestions.map((suggestion, index) => (
              <button
                ...
                className={`w-full text-left text-gray-7 hover:bg-gray-50 flex items-center gap-2 first:rounded-t-lg last:rounded-b-lg ${size === 'sm' ? 'px-3 py-2 text-sm' : 'px-4 py-3 text-sm'
                  }`}
              >
