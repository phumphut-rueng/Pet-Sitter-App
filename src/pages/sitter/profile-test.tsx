...
export default function SitterProfile() {
  ...
  const [profileImage, setProfileImage] = useState(""); // สำหรับแก้ไขรูปโปรไฟล์
  ...
  const fetchProfile = async () => {
    try {
      const response = await axios.get("/api/sitter/profile-test");
  ...
  const updateProfile = async () => {
    setLoading(true);
    try {
      await axios.put("/api/sitter/profile-test", { name, profile_image: profileImage });
...
<h1 className="text-2xl font-bold">Sitter Profile</h1>
      ...
      <div className="border-2 border-gray-300 rounded-md p-4">
        ...
        <p>Profile Image: {profile?.profile_image}</p>
        <p>Roles: {profile?.roles?.join(", ")}</p>
      </div>

      {/* Update Form */}
      <div className="border-2 border-gray-300 rounded-md p-4">
        <h2 className="text-xl font-bold mb-2">Update Sitter Profile</h2>
        <input
