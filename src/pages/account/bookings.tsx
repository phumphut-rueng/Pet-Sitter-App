...
import SelfMessageModal from "@/components/modal/SelfMessageModal";
...
export default function BookingHistoryPage() {
  ...
  const handleSendMessage = (b: BookingCardProps) => {
    // sitterUserId = user.id ของฝั่ง sitter (ไว้คุยกัน)
    if (!b?.sitterUserId) {
      return;
    }
    createChatAndNavigate(b.sitterUserId, () => {
      setShowSelfMessageModal(true); // กัน user แชทหาตัวเอง
    });
  };
  ...
  if (loading) {
    return (
      <AccountPageShell title="Booking History">
        <PetPawLoading message="Loading booking history" size="lg" />
      </AccountPageShell>
    );
  }
  ...
  return (
    <AccountPageShell title="Booking History">
              ...
              {bookings.map((b) => (
                ...
                >
                  <BookingCard
                    ...
                    onReview={() => {
                      setSelectedBooking(b);
                      if (b.hasUserReview && b.userReview) {
                        setReviewData({
                          rating: b.userReview.rating,
                          review: b.userReview.comment,
      ...
      />
      <SelfMessageModal
  isOpen={showSelfMessageModal}
  onClose={() => setShowSelfMessageModal(false)}
/>
